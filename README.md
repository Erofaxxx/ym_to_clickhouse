...
Репозиторий включает три готовых Python-скрипта для работы на macOS (и других ОС):

1. **export_ym_simple.py** ✨ **НОВЫЙ** - упрощённая выгрузка только основных полей (как в .ipynb notebooks)
   - 8 полей для hits, 10 полей для visits
   - Таблицы: `hits_simple`, `visits_simple`
   - Быстрая выгрузка с проверкой доступности полей
   - 📖 [Документация](README_SIMPLE.md)

2. **load_ym_to_clickhouse.py** - полная загрузка данных визитов (~36 полей)
   - Таблица: `ym_visits`
   - Детальный анализ с максимумом данных
   - 📖 [Документация](USAGE_GUIDE.md)

3. **query_clickhouse.py** - выполнение запросов к ClickHouse с красивым форматированием

📊 **Сравнение скриптов:** см. [COMPARISON.md](COMPARISON.md)
